...
int main(int argc, char *argv[]) {
  ...
  #pragma omp parallel for reduction (+:alpha_parallel) num_threads(n)
  for (int iterations = 0; iterations < NUM_ITERATIONS; iterations++) {
    alpha_parallel = 0.0;
    for (int i = 0; i < N; i++) {
  ...
  long double temp_alpha = 0.0;
  time_critical = wall_time();
  # pragma omp parallel shared(a, b, alpha_parallel) private(temp_alpha)
  {
    for (int iterations = 0; iterations < NUM_ITERATIONS; iterations++) {
      alpha_parallel = 0.0;
      temp_alpha = 0.0;
